...
public class Manager extends DutyPosition{
    ...
    public boolean handler(LeaveRequest request) {
        if(supportDays(request.getDays())){
            String msg = String.format("【%s】经理审批【%s】的请假:请假天数【%d】天，请假原因：【%s】",getName()
                    ,request.getName(),request.getDays(),request.getReason());
